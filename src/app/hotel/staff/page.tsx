          ...
          <table className="w-full text-sm">
            <thead>
              <tr>
                ...
                <th className="text-left text-xs font-medium text-muted uppercase tracking-wider pb-3">Role</th>
                <th className="text-left text-xs font-medium text-muted uppercase tracking-wider pb-3">Email</th>
            ...
            <tbody className="divide-y divide-border">
              {staffMembers.map((s) => (
                <tr key={s.id}>
                  <td className="py-3 font-medium text-brand-ink whitespace-nowrap">{s.name}</td>
                  <td className="py-3">
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${roleBadgeColors[s.role]}`}>
                      {roleLabels[s.role]}
                    </span>
                  </td>
                  <td className="py-3 text-muted whitespace-nowrap">{s.email}</td>
                  <td className="py-3">
                    <span
                  ...
                  <td className="py-3">
                    <div className="flex gap-2">
                      <Button size="sm" variant="ghost" className="h-8 px-3 text-xs">Edit</Button>
                      <Button size="sm" variant="ghost" className="h-8 px-3 text-xs text-red-600">Remove</Button>
                    </div>
                  </td>
                </tr>
              ))}
