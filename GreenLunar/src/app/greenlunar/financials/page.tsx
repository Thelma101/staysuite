...
function MenuIcon() {
  return <span className="block h-[2px] w-5 bg-brand-ink shadow-[0_6px_0_0_#00190f,0_-6px_0_0_#00190f]" />;
}
...
function Sidebar({ mobileOpen, onClose }: { mobileOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      <aside className="hidden h-screen w-[177px] shrink-0 border-r border-border bg-white md:block">
        {content}
      </aside>
      <div
        className={cn("fixed inset-0 z-40 md:hidden", mobileOpen ? "pointer-events-auto" : "pointer-events-none")}
      ...
      >
        ...
        <aside
          className={cn(
            "absolute left-0 top-0 h-full w-[220px] max-w-[80%] border-r border-border bg-white shadow-card transition-transform",
  ...
  return (
    <header className="flex h-[70px] items-center justify-between border-b border-border bg-white px-4 md:px-7">
      ...
      <div className="flex items-center gap-3">
        <div className="size-10 rounded-full bg-brand-green/20" />
      </div>
    </header>
  );
}
...
export default function GreenLunarFinancialsPage() {
  ...
  useEffect(() => {
    Promise.all([fetchWalletSummary(), fetchTransactions()])
      .then(([w, t]) => {
        setWallet(w);
        setTransactions(t);
      })
  ...
  const handleWithdrawContinue = () => {
    if (withdrawStep === "account") {
      const check = validateAccountNumber(accountNumber);
      if (!check.ok) {
        setWithdrawError(check.message);
        return;
      }
      setWithdrawError(null);
    ...
    <div className="flex min-h-screen bg-[#f7f9fb]">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex-1 px-4 py-6 md:px-7 md:py-8">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {loading
              ? Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="h-24 animate-pulse rounded-[10px] bg-white shadow-sm" />
                ))
              : statCards.map((stat) => (
          ...
          <div
            className="mt-6 flex flex-col items-center justify-center rounded-[14px] px-6 py-10"
            style={{ background: "linear-gradient(145deg, #cd9c33, #b99a38, #188b5c)" }}
          >
            <p className="text-sm font-medium text-white/80">Total balance</p>
            <p className="mt-2 text-3xl font-bold text-white md:text-4xl">
              {loading ? "…" : <Naira>{(wallet?.balance ?? "₦0").replace(/^₦/, "")}</Naira>}
            </p>
          </div>
          ...
          <div className="mt-4 flex items-center justify-between rounded-[10px] bg-white px-6 py-5 shadow-sm">
            ...
            <Button type="button" size="sm" onClick={() => setWithdrawOpen(true)}>
              Withdraw
            </Button>
          </div>
          ...
          <div className="mt-8">
            ...
            {loading ? (
              ...
              <p className="rounded-[10px] bg-white px-5 py-8 text-center text-sm text-muted shadow-sm">
                No transactions yet.
              </p>
            ) : (
                ...
                {transactions.map((tx) => (
                  ...
                  >
                    ...
                    <div className="flex min-w-0 flex-1 flex-col sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <p className="mt-1 text-sm font-bold text-red-500 sm:mt-0">
                        -<Naira>{tx.amount.replace(/^₦/, "")}</Naira>
                      </p>
                    </div>
                  </button>
                ))}
      ...
      <SlidePanel
        ...
        title={withdrawStep === "success" ? "Withdrawal Complete" : "Withdraw Money"}
      >
        ...
        {withdrawStep === "amount" && (
          <div className="flex flex-col gap-6">
            ...
            <p className="text-sm text-muted">
              Withdrawing to account ending{" "}
              <span className="font-semibold text-brand-ink">{accountNumber.slice(-4)}</span>
            </p>
        ...
        {withdrawStep === "success" && (
          <div className="flex flex-col items-center gap-6 text-center">
            ...
            <p className="text-sm text-muted">
              Your withdrawal of <Naira>{amount}</Naira> has been submitted successfully.
            </p>
            <Button type="button" fullWidth className="h-12" onClick={closeWithdraw}>
              Done
            </Button>
          </div>
        )}
        ...
        {selectedTx && (
          <div className="flex flex-col gap-4 text-sm">
            <div>
              ...
              <p className="font-semibold text-brand-ink">{selectedTx.name}</p>
            </div>
            <div>
              <p className="text-muted">Bank</p>
              <p className="font-semibold text-brand-ink">{selectedTx.bank}</p>
            </div>
            <div>
              <p className="text-muted">Date</p>
              <p className="font-semibold text-brand-ink">{selectedTx.date}</p>
            </div>
            {selectedTx.reference && (
              <div>
                <p className="text-muted">Reference</p>
                <p className="font-semibold text-brand-ink">{selectedTx.reference}</p>
              </div>
            )}
            <div>
              <p className="text-muted">Amount</p>
              <p className="text-lg font-bold text-red-500">
                -<Naira>{selectedTx.amount.replace(/^₦/, "")}</Naira>
              </p>
            </div>
          </div>
        )}
